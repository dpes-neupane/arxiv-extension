document.addEventListener('DOMContentLoaded', async() => {
    ...
    function loadArXivTabs() {
        chrome.runtime.sendMessage({ action: 'getArXivTabs' },
            (response) => {
    ...
    function parseArxivXml(xmlString) {
        ...
        return Array.from(entries).map(entry => {
            ...
            const getTitle = () => {
                ...
                return titleElement ? titleElement.textContent.replace(/\n/g, '').trim() : 'Untitled';
            };
            ...
            return {
                ...
                summary: getSummary(),
                authors: getAuthors(),
                published: entry.getElementsByTagName('published')[0] ?.textContent || null,
                updated: entry.getElementsByTagName('updated')[0] ?.textContent || null,
                pdfUrl: links.pdf || links.alternate || null,
                primaryCategory: entry.getElementsByTagName('arxiv:primary_category')[0] ?.getAttribute('term') || null,
                categories: Array.from(entry.getElementsByTagName('category')).map(cat =>
        ...
        for (i = 0; i < checkBoxes.length; i++) {
            ...
            if (checkBoxes[i].checked) {
                paperInfo = {
                    ...
                    summary: checkBoxes[i].getAttribute("paper-summary").replaceAll("\n", " ")
                }
                data.push(paperInfo);
    ...
    function renderTabsList(tabs) {
        ...
        if (tabs.length === 0) {
            tabsListElement.innerHTML = '<li class="no-tabs">No arXiv tabs found</li>';
            return;
        }
